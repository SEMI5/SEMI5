...
/**
 * Servlet implementation class InsertSFileServlet
 */
@WebServlet("/insert.sh")
public class InsertSFileServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertSFileServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			// enctype이 multipart/form-data로 전송되었는지 확인!
				if(ServletFileUpload.isMultipartContent(request)) {
					...
					String savePath = root + "share_uploadFiles/";
					...
					// 3_2. DB에 보낼 Share 객체와 Attachment 리스트 생성
					Share b = new Share();
					b.setbTitle(title);
					...
					int result = new SFileService().insertShare(b,fileList);
					
					if(result > 0) {
						response.sendRedirect("list.sh");  //DB에 갔다가 갱신된 데이터를 가져와야함
						...
						request.setAttribute("msg", "자료실 게시판 등록 실패!!");
